...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const cosA = scalarMultiply / (length1 * length2);
...
function isPrime(n) {
  if (n < 4) return true;
  function recursiveCalc(num, divider) {
    if (divider === 1) return true;
    if (num % divider === 0) return false;
    return recursiveCalc(n, divider - 1);
  }

  return recursiveCalc(n, n - 1);
}
...
function getFibonacciNumber(index) {
  ...
  let i = 3;
  while (i <= index) {
    const fibonachiNum = prev + current;
    prev = current;
    current = fibonachiNum;
    i += 1;
  }
  return current;
}
